...
class BaseAgent(object):
    def __init__(self, config, shapeOfState, sizeOfActions):
        self.model=None
        ...
        self.action_selections = [0 for _ in range(sizeOfActions)]
    ...
    def save_w(self, modelName):
        torch.save(self.model.state_dict(), './saved_agents/model_' + modelName + '.pth')
        torch.save(self.optimizer.state_dict(), './saved_agents/optim_' + modelName + '.pth')
    
    def load_w(self, modelName):
        fname_model = "./saved_agents/model_" +  modelName + ".pth"
        fname_optim = "./saved_agents/optim_" +  modelName + ".pth"

        if os.path.isfile(fname_model):
            self.model.load_state_dict(torch.load(fname_model, map_location='cpu'))
            self.target_model.load_state_dict(self.model.state_dict())    

        if os.path.isfile(fname_optim):
            self.optimizer.load_state_dict(torch.load(fname_optim, map_location='cpu'))
