...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 26 01:35:23 2019
...
class ReplayBuffer:
    ...
    def __init__(self, action_size, buffer_size, batch_size):
        ...
        self.experience = namedtuple("Experience", field_names=["state", "action", "reward", "next_state", "next_action"])
        
    def add(self, state, action, reward, next_state, next_action):
        """Add a new experience to memory."""
        e = self.experience(state, action, reward, next_state, next_action)
    ...
    def sample(self):
        ...
        next_actions = torch.from_numpy(np.vstack([e.next_action for e in experiences if e is not None])).float().to(device)
